...
function initializeEventListeners() {
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener("click", (e) => {
      ...
      const sessionName = meta ? meta.name : "Theseus Chat";
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute(
          "placeholder",
          w < PLACEHOLDER_HIDE_WIDTH ? "" : "Message Theseus...",
        );
      }
    ...
    for (const node of nodes) {
      ...
      if (
        ...
        node.parentElement.closest(
          ".sources-section, .thinking-toggle, .memory-used-pill",
        )
      )
        continue;
  ...
  (function initUIVisibility() {
    ...
    try {
      ...
      console.error("Modal drag init error:", e);
    }
  })();
  ...
  if (brandBtn) {
    brandBtn.addEventListener("click", async () => {
      ...
      if (withModel.length > 0) {
        sessionModule.createDirectChat(
          withModel[0].endpoint_url,
          withModel[0].model,
          withModel[0].endpoint_id,
        );
        return;
      }
      // No models at all — show welcome screen
    ...
    railDelete.addEventListener("click", async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/session/${currentId}/archive`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          },
